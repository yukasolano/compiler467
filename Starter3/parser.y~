...
program
  : scope 
      { yTRACE("program -> scope\n"); ast = ast_allocate(NESTED_SCOPE_NODE,$1);} 
  ;
...
type
  : INT_T
      { yTRACE("type -> INT_T \n"); $$ = ast_allocate(TYPE_NODE, "int", NULL); }
  | IVEC_T
      { yTRACE("type -> IVEC_T \n"); $$ = ast_allocate(TYPE_NODE, "ivec", $1); }
  | BOOL_T
      { yTRACE("type -> BOOL_T \n"); $$ = ast_allocate(TYPE_NODE, "bool", NULL); }
  | BVEC_T
      { yTRACE("type -> BVEC_T \n"); $$ = ast_allocate(TYPE_NODE, "bvec", $1); }
  | FLOAT_T
      { yTRACE("type -> FLOAT_T \n"); $$ = ast_allocate(TYPE_NODE, "float", NULL); }
  | VEC_T
      { yTRACE("type -> VEC_T \n"); $$ = ast_allocate(TYPE_NODE, "vec", $1); }
  ;
